...
当`if`条件成立需要执行多条语句时，只需确保这些语句保持相同的缩进即可。具体来说，连续的具有相同缩进的语句会被视为同一个**代码块**，作为一个执行单元。虽然缩进可以使用任意数量的空格，但Python社区普遍采用**4个空格**作为标准。强烈建议开发者**避免使用制表键（Tab键）进行缩进**。如果你习惯使用Tab键，可以在代码编辑器中设置自动将Tab转换为4个空格，大多数现代编辑器（包括PyCharm）都默认支持这一功能。

另外值得一提的是，Python在条件判断语法上也更加简洁优雅。以BMI判断为例，其他语言通常需要写成`bmi >= 18.5 and bmi < 24`这样的形式，而Python支持更直观的`18.5 <= bmi < 24`写法。虽然两种写法在Python中都是可行的，但后者无疑更加简洁明了。为了帮助理解这种差异，下面会展示一个等效的Java代码示例。即使你不熟悉Java语法，也可以通过对比感受到Python语法的优雅之处。
